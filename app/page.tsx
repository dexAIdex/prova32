...
export default function Home() {
  ...
  const getTokenPrice = async (): Promise<bigint> => {
    try {
      const provider = new JsonRpcProvider("https://bsc-dataseed.binance.org/");
      ...
      const tokenPrice = await presaleContract.tokenPrice();
      console.log("TokenPrice:", tokenPrice.toString());
      return tokenPrice;
    } catch (err) {
      console.error("Error getting tokenPrice:", err);
      return BigInt(0);
    }
  };
...
if (tokenPrice === BigInt(0)) {
  alert("Errore nel recuperare il prezzo del token");
  return;
}

// Calcolo quanti token riceverai per debug:
const amountInWei = parseEther(bnbAmount);
const tokensToBuy = (amountInWei * BigInt(1e18)) / tokenPrice;
